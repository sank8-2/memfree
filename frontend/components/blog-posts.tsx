            ...
            <section>
                ...
                <article className="relative grid grid-cols-1 gap-6 md:grid-cols-2">
                    ...
                    <div className="flex flex-col justify-center">
                        <h3 className="mb-2 text-balance font-heading text-2xl md:text-4xl">{posts[0].title}</h3>
                        {posts[0].description && <p className="text-balance text-muted-foreground md:text-lg">{posts[0].description}</p>}
                ...
                <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    {posts.slice(1).map((post) => (
                        <article key={post._id} className="group relative flex flex-col space-y-2">
                            {post.image && (
                                <Image alt={post.title} src={post.image} width={804} height={452} className="rounded-md border bg-muted transition-colors" />
                            )}
                            <h2 className="line-clamp-1 font-heading text-2xl">{post.title}</h2>
                            {post.description && <p className="line-clamp-1 text-muted-foreground">{post.description}</p>}
